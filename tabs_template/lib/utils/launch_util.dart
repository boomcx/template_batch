...
class LaunchUtil {
  ...
  static Future callPhone(String? number) async {
    return Get.dialog(
      ...
      barrierColor: Get.context!.appColors.barrierColor,
      useSafeArea: false,
    );
  }

  /// 打开网页
  static Future openUrl(String url,
      {LaunchMode model = LaunchMode.externalApplication}) async {
    try {
      Uri launchUri;
      var path = url;
      if (url.endsWith('/')) {
      ...
      launchUri = Uri.parse(
          '${splitList.first == 'https' ? 'https' : 'http'}://${splitList.last}');
      // launchUri = Uri(
